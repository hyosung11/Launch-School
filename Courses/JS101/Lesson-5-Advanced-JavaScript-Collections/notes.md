...
From the problem description, it is clear that we must return a subset of the objects in the outer array, so `filter` should hopefully be an obvious method choice to call on that array. The key then is to figure out how to carry out the selection.
