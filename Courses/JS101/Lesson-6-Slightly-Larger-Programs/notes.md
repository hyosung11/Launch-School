...
As usual, we'll take the most obvious, non-clever approach first. To see whether the human player is about to win, we need to iterate through the winning lines array that we described in the detectWinner function. Since we need that same array, let's move it out of the function and define it as a constant:
...
Since `WINNING_LINES` is an array of 3-element arrays, we can iterate through it and pass each element (a 3-element array) to a function to see whether any of those lines are "at risk." That means we need a function to inspect the 3-element array and tell us whether the human player has a potential winning move; it should return the move as the number of a square that the human can use to win the game.
...
Line 2 returns an array of the values that the `board` object has for the numbers in the given line. Thus, if the given line is `[1, 2, 3]` and the board has `X`'s on square 2, and 3 and a space character on square 1, the return value will be `[' ', 'X', 'X']`. Line 4 then checks whether the player marker occurs twice in the array, and line 5 determines the id of the remaining square, provided it is unused. If it is unused, we return its key to the caller. Otherwise, we return null.

Once you've defined `findAtRiskSquare`, you can modify `computerChoosesSquare` like this:
...
Finally, after we verify that `findAtRiskSquare` works, we can use our constants there as well:
...
First, we'll make some slight modifications to `findAtRiskSquare`. In the defensive mode, we want to look for occurrences of the player's marker; in offensive mode, we want to look for occurrences of the computer's marker. So, in both cases, we'll pass the marker of interest to the function:
...
Now that we're passing in the marker, we can use `findAtRiskSquare` for both defense and offense. Only when there are no defensive or offensive moves will the computer pick a random square.
...
Consider what happens when the user enters something that begins with `n` or `y` but isn't unambiguously a yes/no response. For instance, `yellow` or `narwhal`. What should the program do in those cases?
...
Complete Assignment 8. TTT Bonus Features
