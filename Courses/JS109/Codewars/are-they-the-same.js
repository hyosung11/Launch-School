...
//   let squared = array1.map((value) => value * value).sort((a, b) => a - b);
//   let sorted = array2.sort((a, b) => a - b);

//   for (let idx = 0; idx < squared.length; idx++) {
//     for (let jdx = 0; jdx < sorted.length; jdx++) {
//       if (squared[idx] === sorted[jdx]) return true;
//     }
//   }
//   return false;
// }
...
//   return arr1.map(element => element * element).sort().toString() === arr2.sort().toString();
// }
...
function comp(array1, array2) {
  ...
  array1.sort((a, b) => a - b);
  array2.sort((a, b) => a - b);
  return array1.map(num => num * num).every((value, idx) => value === array2[idx]);
}
