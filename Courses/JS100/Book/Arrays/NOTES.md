...
* The `slice` method (not the `splice`) extracts and returns a portion of the array.
* It takes two optional arguments
  * The first is the index at which extraction begins.
  * The second is where extraction ends:
...
* If you omit the second argument, `slice` returns the rest of the array starting with the index given by the first argument. 
* With the second argument, it returns the elements up to but _excluding_ that index. 
